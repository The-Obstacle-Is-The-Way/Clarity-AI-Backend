...
from app.domain.entities.biometric_twin_enhanced import BiometricDataPoint, BiometricTwin, BiometricType, BiometricSource
...
class BiometricIntegrationService:
    ...
    def get_or_create_biometric_twin(self, patient_id: UUID) -> BiometricTwin:
        ...
        try:
            ...
            logger.info(f"Successfully created and saved new BiometricTwin for patient_id: {str_patient_id}")
            return new_twin
        except Exception as e:
            logger.error(f"Failed to get or create biometric twin for patient {patient_id}: {e!s}", exc_info=True)
            raise DomainError(f"Failed to get or create biometric twin for patient {patient_id}: {e!s}")
    ...
    def add_biometric_data(
        ...
        try:
            ...
            logger.info(f"Added biometric data ({biometric_type_enum.value}) for patient {patient_id}")
            return data_point
        except ValueError as ve: 
            logger.error(f"ValueError in add_biometric_data for patient {patient_id}: {ve!s}", exc_info=True)
            raise ve
        except Exception as e:
            logger.error(f"Failed to add biometric data for patient {patient_id}: {e!s}", exc_info=True)
            raise DomainError(f"Failed to add biometric data for patient {patient_id}: {e!s}")
    
    def batch_add_biometric_data(
        self,
        patient_id: UUID,
        data_points_dicts: list[dict] 
        ...
        try:
            ...
            for i, point_data in enumerate(data_points_dicts):
                ...
                if not all([isinstance(data_type_str, str), value is not None, isinstance(source_str, str)]):
                ...
                if isinstance(point_timestamp_input, str):
                    try:
                        point_timestamp = datetime.fromisoformat(point_timestamp_input.replace("Z", "+00:00"))
                    except ValueError:
            ...
            if created_points: 
                self.biometric_twin_repository.save(twin)
            logger.info(f"Batch added {len(created_points)} biometric data points for patient {patient_id}")
            return created_points
        except ValueError as ve: 
            logger.error(f"ValueError in batch_add_biometric_data for patient {patient_id}: {ve!s}", exc_info=True)
            raise ve
        except Exception as e:
            logger.error(f"Failed to batch add biometric data for patient {patient_id}: {e!s}", exc_info=True)
    ...
    def get_biometric_data(
        ...
        try:
            twin = self.biometric_twin_repository.get_by_patient_id(patient_id)
            if not twin:
                logger.info(f"No BiometricTwin found for patient {patient_id} in get_biometric_data.")
                return []
            ...
            if data_type:
                try:
                    ...
                    logger.warning(f"Invalid data_type filter '{data_type}' in get_biometric_data. Returning empty list.")
                    return [] 
            ...
            if source:
                try:
                    ...
                    logger.warning(f"Invalid source filter '{source}' in get_biometric_data. Returning empty list.")
                    return []
            ...
            if biometric_type_filter:
                ...
                if hasattr(twin, 'timeseries_data') and isinstance(twin.timeseries_data, dict):
                    ...
                    logger.warning(f"BiometricTwin for patient {patient_id} missing or has malformed timeseries_data attribute.")
            ...
            if biometric_source_filter: 
                filtered_points = [dp for dp in filtered_points if dp.source == biometric_source_filter]
            
            return sorted(filtered_points, key=lambda dp: dp.timestamp)
        except Exception as e:
            logger.error(f"Failed to retrieve biometric data for patient {patient_id}: {e!s}", exc_info=True)
    ...
    def analyze_trends(
        ...
        try:
            ...
            for dp in data_points:
                ...
                elif isinstance(dp.value, dict) and "value" in dp.value and isinstance(dp.value["value"], (int,float)):
            ...
            logger.error(f"Failed to analyze trends for patient {patient_id}, data_type {data_type}: {e!s}", exc_info=True)
    ...
    def detect_correlations(
        ...
        (Simplified placeholder implementation)
        """
        logger.info(f"Detecting correlations for patient {patient_id}, primary_data_type {primary_data_type}")
        correlations = {}
    ...
    def connect_device(
        ...
        """
        logger.info(f"Attempting to connect device {device_id} ({device_type}) for patient {patient_id}")
        try:
            twin = self.get_or_create_biometric_twin(patient_id)
            ...
            self.add_biometric_data( 
                ...
                value={"event": "device_connected", "device_id": device_id, "device_type": device_type},
                source="system_service", 
                metadata=event_metadata
            )
            
            logger.info(f"Device connection event recorded for device {device_id}, patient {patient_id}")
            return True 
        except Exception as e:
            logger.error(f"Failed to connect device {device_id} for patient {patient_id}: {e!s}", exc_info=True)
    ...
    async def disconnect_device(
        ...
        try:
            ...
            if not twin:
                logger.warning(f"BiometricTwin not found for patient {patient_id} during disconnect_device.")
                return False
            ...
            logger.info(f"Device disconnection event recorded for device {device_id}, patient {patient_id}")
            return True
        except Exception as e:
            logger.error(f"Failed to disconnect device {device_id} for patient {patient_id}: {e!s}", exc_info=True)
