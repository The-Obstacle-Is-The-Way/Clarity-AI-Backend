...
class NeurotransmitterVisualizationPreprocessor:
    ...
    def _precompute_time_series_geometry(
        self, cascade_data: dict[BrainRegion, list[float]]
    ) -> dict[str, Any]:
        ...
        for i, source in enumerate(regions):
            for target in regions[i + 1 :]:
                if (
                    source in self._brain_region_coordinates
                    and target in self._brain_region_coordinates
                ):
    ...
    def _precompute_detailed_geometry(
        self, cascade_data: dict[BrainRegion, dict[Neurotransmitter, float]]
    ) -> dict[str, Any]:
    ...
    def preprocess_sequence_for_visualization(
        self, sequence: TemporalSequence, downsample_factor: int = 1
    ) -> dict[str, Any]:
        """
    ...
    def process_neurotransmitter_levels(
        self,
        ...
        if "comparison_metrics" in comparison:
            ...
            if "most_significant" not in metrics and effects:
                ...
                if significant_effects:
                    ...
                    metrics["most_significant"] = (
                        effects[0].neurotransmitter.value if effects else None
                    )

            # Add largest effect if missing
            if "largest_effect" not in metrics and effects:
                metrics["largest_effect"] = max(
