...
class BiometricAlertNotificationService(AlertObserver):
    ...
    def _create_hipaa_compliant_message(
        self, alert: BiometricAlert, channel: NotificationChannel
    ) -> str:
        """
    ...
    async def _send_sms_notifications(
        ...
        for recipient in recipients:
            if "phone" in recipient:
                await self.notification_service.send_sms(recipient["phone"], message)

    async def _send_email_notifications(
        self, message: str, recipients: list[dict[str, Any]], alert: BiometricAlert
    ) -> None:
        """
        ...
        for recipient in recipients:
            if "provider_id" in recipient:
                await self.notification_service.send_push_notification(
                    recipient["provider_id"], "Biometric Alert", message
                )
