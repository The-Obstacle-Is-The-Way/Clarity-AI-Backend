...
class TemporalKnowledgeGraph:
    ...
    def _dfs_chain(
        self,
        current_edge: KnowledgeGraphEdge,
...
class BayesianBeliefNetwork:
    ...
    def add_dependency(self, child: str, parent: str) -> None:
        ...
        if child not in self.variables or parent not in self.variables:
            raise ValueError(
                f"Both child ({child}) and parent ({parent}) must exist in the network"
            )
    ...
    def get_belief_state(self) -> dict[str, dict[str, float]]:
        ...
        for var in ordered_vars:
            ...
            for parent_config in self._generate_parent_configs(var, beliefs):
                ...
                if (
                    var not in self.conditional_probabilities
                    or config_str not in self.conditional_probabilities[var]
                ):
    ...
    def _generate_parent_configs(
        self, var: str, beliefs: dict[str, dict[str, float]]
    ) -> list[dict[str, str]]:
    ...
    def _parent_config_probability(
        self, config: dict[str, str], beliefs: dict[str, dict[str, float]]
    ) -> float:
