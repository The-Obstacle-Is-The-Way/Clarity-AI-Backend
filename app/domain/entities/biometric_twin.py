...
class BiometricDataPoint(BaseModel):
    ...
    value: float = Field(..., description="Numerical value of the data point")
    timestamp: datetime = Field(..., description="Time when the data was recorded")
    source: str = Field(
        ..., description="Source of the data (e.g., apple_watch, fitbit, manual_entry)"
    )
    metadata: dict[str, Any] | None = Field(
        None, description="Additional contextual information about the data point"
    )
    confidence: float | None = Field(
        None, description="Confidence score for the data point (0.0 to 1.0)"
    )
...
class BiometricTwinState(BaseModel):
    ...
    data_points: dict[str, list[BiometricDataPoint]] = Field(
        default_factory=dict, description="Dictionary of data points by type"
    )
    derived_metrics: dict[str, Any] = Field(
        default_factory=dict, description="Metrics derived from the raw data points"
    )
    ...
    risk_scores: dict[str, float] = Field(
        default_factory=dict, description="Risk scores for various clinical concerns"
    )
