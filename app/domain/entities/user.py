...
class User(BaseModel):
    ...
    hashed_password: str = Field(
        default="hashed_password_for_testing",
    ...
    account_locked_until: datetime | None = Field(
        default=None, description="When account lockout expires"
    )

    # Roles and permissions
    # Maintaining compatibility with existing code that expects 'roles' array
    roles: list[str | object] = Field(
        default_factory=list, description="User roles for authorization"
    )
    ...
    preferences: dict | None = Field(
        default=None, description="User preferences (UI settings, etc.)"
    )

    # Audit and compliance
    last_password_reset_request: datetime | None = Field(
        default=None, description="When password reset was last requested"
    )
    terms_agreed_at: datetime | None = Field(
        default=None, description="When the user agreed to the terms"
    )
    privacy_policy_agreed_at: datetime | None = Field(
        default=None, description="When the user agreed to the privacy policy"
    )
