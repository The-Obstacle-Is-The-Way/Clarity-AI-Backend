...
async def generate_therapeutic_response(
    conversation_history: List[Dict[str, str]] = Body(..., description="Conversation history with user and therapist messages"),
    user_id: str = Body(..., description="User ID for logging and personalization"),
    service: MentaLLaMAInterface = Depends(get_mentallama_service),
    current_user = Depends(get_current_active_user)
) -> Dict[str, Any]:
...
async def assess_wellness_dimensions(
    ...
    user_id: str = Body(..., description="User ID for logging and personalization"),
    dimensions: List[str] = Body(default=None, description="Specific wellness dimensions to analyze"),
    service: MentaLLaMAInterface = Depends(get_mentallama_service),
    current_user = Depends(get_current_active_user)
) -> Dict[str, Any]:
    """
