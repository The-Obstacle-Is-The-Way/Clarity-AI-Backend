...
async def get_comprehensive_insights(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error generating insights: {e}") from e
    except Exception as e:
        # Log the unexpected error
...
async def analyze_clinical_text(
    ...
    try:
        ...
        if analysis_response is None:
             ...
             raise ModelExecutionError(f"Failed to initiate clinical text analysis for patient {patient_id}")
        return analysis_response
    except ModelExecutionError as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error during text analysis: {e}") from e
    except Exception as e:
        # Log the unexpected error
