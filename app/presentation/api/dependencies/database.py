...
async def get_session_factory_from_request_state(request: Request) -> async_sessionmaker[AsyncSession]:
    logger.debug("DEPENDENCY:get_session_factory_from_request_state: Attempting to retrieve actual_session_factory from request.state")
    
    if not hasattr(request, 'state'):
        logger.error("DEPENDENCY:get_session_factory_from_request_state: request has NO 'state' attribute!")
    ...
    if factory is None:
        logger.error("DEPENDENCY:get_session_factory_from_request_state: 'actual_session_factory' NOT FOUND on request.state.")
        ...
        if hasattr(request.state, '_state') and isinstance(request.state._state, dict):
            ...
            req_state_content_for_log = {k: type(v) for k, v in request.state.__dict__.items() if not k.startswith('_')}
        else:
            req_state_content_for_log = dir(request.state)
        logger.error(f"DEPENDENCY:get_session_factory_from_request_state: Content of request.state: {req_state_content_for_log}")
        raise RuntimeError("actual_session_factory not found on request.state")
        
    logger.debug(f"DEPENDENCY:get_session_factory_from_request_state: Successfully retrieved actual_session_factory: {type(factory)}")
    return factory
...
async def get_async_session_utility(
    ...
    logger.debug(f"DEPENDENCY:get_async_session_utility: Received factory: {type(factory)}. Creating session.")
    ...
    if not factory:
        logger.error("DEPENDENCY:get_async_session_utility: Session factory is None, cannot create session.")
    ...
    try:
        async with factory() as session:
            logger.debug(f"DEPENDENCY:get_async_session_utility: Session {id(session)} created. Yielding session.")
            yield session
            logger.debug(f"DEPENDENCY:get_async_session_utility: Session {id(session)} context exited.")
    except SQLAlchemyError as e:
        logger.error(f"DEPENDENCY:get_async_session_utility: SQLAlchemy error during session management: {e}", exc_info=True)
        ...
        logger.error(f"DEPENDENCY:get_async_session_utility: Unexpected error during session management: {e}", exc_info=True)
...
async def get_db_session(
    ...
    logger.debug(f"DEPENDENCY:get_db_session: Yielding session {id(session)} from get_async_session_utility.")
...
def get_repository(repo_type: type[T]) -> Callable[[AsyncSession], T]:
    ...
    async def _get_repo_with_session(session: AsyncSession = Depends(get_db)) -> T: # Depends on the simplified get_db
        """
