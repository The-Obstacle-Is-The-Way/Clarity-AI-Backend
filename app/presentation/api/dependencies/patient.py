...
async def get_patient_id(
    ...
    if is_patient_role and not (is_clinician_role or is_admin_role): # Patient ONLY
        ...
        logger.info(f"Patient user {current_user.username} accessing their own data for patient_id: {validated_patient_uuid}")
    ...
    logger.info(f"User {current_user.username} (Roles: {current_user.roles}) authorized to attempt fetch for patient_id: {validated_patient_uuid}")
