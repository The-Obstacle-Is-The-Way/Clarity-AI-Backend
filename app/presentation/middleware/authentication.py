...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    def __init__(
        ...
        public_paths_str = (
            str(self.public_paths)
            if len(str(self.public_paths)) < 80
            else f"{len(self.public_paths)} paths"
        )
        regex_paths_str = (
            str(self.public_path_regexes)
            if len(str(self.public_path_regexes)) < 80
            else f"{len(self.public_path_regexes)} patterns"
        )
    ...
    async def _validate_and_prepare_user_context(
        ...
        try:
            ...
            if "inactive" in user_str:
                logger.warning(
                    f"User {user_id} has 'inactive' in string representation: {user_str}"
                )
                is_active = False
            ...
            try:
                        ...
                        for role in domain_roles:
                            ...
                            # Otherwise try to convert to UserRole
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"Invalid role string: {role}, using PATIENT as default"
                                    )
                ...
                username = (
                    str(domain_user.username)
                    if hasattr(domain_user, "username")
                    else "unknown_user"
                )
    ...
    async def dispatch(
        ...
        try:
            ...
            logger.debug(
                f"User authenticated successfully: {user_context.id} with scopes: {scopes}"
            )
