...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "standalone: Tests that have no external dependencies"
    )
    config.addinivalue_line(
        "markers", "db_required: Tests that require database connections"
    )
    config.addinivalue_line(
        "markers",
    ...
    config.addinivalue_line(
        "markers", "phi: Protected Health Information related tests"
    )
    config.addinivalue_line("markers", "integration: Integration tests")
    config.addinivalue_line("markers", "e2e: End-to-end tests")
    config.addinivalue_line(
        "markers", "security: Tests specifically validating security features"
    )
    config.addinivalue_line(
        "markers", "slow: Tests that take longer than 1 second to execute"
    )
    config.addinivalue_line(
    ...
    config.addinivalue_line(
        "markers", "network_required: Tests that require network connections"
    )
