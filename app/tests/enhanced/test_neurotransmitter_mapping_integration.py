...
async def test_neurotransmitter_mapping_initialization(
    ...
    brain_regions = {profile.brain_region for profile in mapping.receptor_profiles}
    ...
    neurotransmitters = {profile.neurotransmitter for profile in mapping.receptor_profiles}
...
async def test_add_custom_receptor_profile(
    ...
    await digital_twin_service.add_receptor_profile(patient_id=patient_id, profile=custom_profile)

    # Get the updated mapping
    updated_mapping = await digital_twin_service.get_neurotransmitter_mapping(patient_id=patient_id)

    # Find the custom profile
    matching_profiles = [
        p
        for p in updated_mapping.receptor_profiles
        if (
            p.brain_region == BrainRegion.PREFRONTAL_CORTEX
            and p.neurotransmitter == Neurotransmitter.SEROTONIN
            and p.receptor_subtype == ReceptorSubtype.SEROTONIN_5HT2A
        )
    ]
...
async def test_simulate_neurotransmitter_cascade(
    ...
    assert (
        first_point["neurotransmitter_levels"][Neurotransmitter.SEROTONIN.value] > 0.4
    )  # Baseline + increase
    ...
    assert (
        last_point["neurotransmitter_levels"][Neurotransmitter.DOPAMINE.value] != 0.5
...
async def test_analyze_neurotransmitter_interactions(
    ...
    interactions = await digital_twin_service.analyze_neurotransmitter_interactions(
        patient_id=patient_id, brain_region=BrainRegion.PREFRONTAL_CORTEX
    )
...
async def test_predict_medication_effects(
    ...
    prediction = await digital_twin_service.predict_medication_effects(
        patient_id=patient_id, medication=medication_data, prediction_timeframe_days=28
    )
...
async def test_integrated_neurotransmitter_mapping_with_visualization(
    ...
    assert any(
        nt["id"] == Neurotransmitter.SEROTONIN.value for nt in brain_viz["neurotransmitters"]
    )
