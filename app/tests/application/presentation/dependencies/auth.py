...
async def get_current_user(
    ...
    try:
        ...
        raise HTTPException(
            ...
            headers={
                "WWW-Authenticate": 'Bearer error="invalid_grant", error_description="Authentication failed."'
            },  # Consider error type
        )
    except Exception as e:
        # Catch unexpected errors during validation
        logger.error(
            f"Unexpected error during token validation/user retrieval: {e}",
            exc_info=True,
        )
        raise HTTPException(
...
def require_role(required_role: str):  # Use simple string role for this version
    ...
    async def role_checker(
        ...
        if required_role not in user_roles:
            logger.warning(
                f"User {current_user.id} with roles {user_roles} tried accessing resource requiring {required_role}"
            )
            raise HTTPException(
