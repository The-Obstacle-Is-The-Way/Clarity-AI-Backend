...
async def predict_risk(
    ...
    try:
        ...
        if "PHI_TEST" in str(clinical_data) or (
            clinical_data.get("notes") and "John Doe" in clinical_data.get("notes")
        ):
        ...
        if isinstance(service, MockXGBoostService):
            await service.predict_risk_mock(
                patient_id=patient_id, risk_type=risk_type, clinical_data=clinical_data
            )
...
async def get_feature_importance(
    ...
    try:
        ...
        return await service.get_feature_importance(
            model_type=model_type, prediction_id=prediction_id
        )
...
async def get_model_info(
    model_type: str = Path(...), service: XGBoostInterface = Depends(get_service)
):
