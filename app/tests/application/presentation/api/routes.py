...
def setup_routers() -> APIRouter:
    ...
    try:
        ...
        main_api_router.include_router(
            auth_ep_router, prefix="/auth", tags=["Authentication"]  # Remove v1 prefix
        )
        print(
            f"Successfully included auth router. Routes: {[route.path for route in auth_ep_router.routes]}"
        )
    ...
    try:
        if is_testing:
            ...
            try:
                ...
                print(
                    f"Successfully included test_xgboost_router with routes: {[route.path for route in test_xgboost_router.routes]}"
                )
            ...
            main_api_router.include_router(
                get_router("xgboost"), prefix="/xgboost", tags=["XGBoost ML Services"]
            )
    except (ModuleNotFoundError, AttributeError) as e:
    ...
    try:
        main_api_router.include_router(
            get_router("mentallama"), prefix="/mentallama", tags=["MentaLLaMA"]
        )
    except (ModuleNotFoundError, AttributeError):
