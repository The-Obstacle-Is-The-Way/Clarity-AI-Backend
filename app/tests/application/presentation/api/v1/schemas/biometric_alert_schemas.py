...
class AlertRuleTemplateListResponse(BaseModel):
    ...
    templates: list[AlertRuleTemplateResponse] = Field(
...
class BiometricAlertResponse(BaseModel):
    ...
    acknowledged_by: UUID | None = Field(
        None, description="ID of the user who acknowledged the alert"
    )
    data_point: dict[str, Any] = Field(
        ..., description="Biometric data point that triggered the alert"
    )
