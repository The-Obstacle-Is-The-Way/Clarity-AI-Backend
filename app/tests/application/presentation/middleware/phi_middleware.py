        ...
        try:
            ...
            if contains_phi:
                if self.audit_mode:
                    ...
                    logger.warning(
                        "PHI detected in request to %s (audit mode)",
                        request.url.path
                    )
                else:
                    # Preprocess to protect whitelisted content
                    preprocessed_data = self._preprocess_response_for_whitelist(data, request.url.path)
                    ...
                    logger.info(
                        "Sanitized PHI in request to %s for logging",
                        request.url.path
                    )
        ...
        try:
            ...
            if contains_phi:
                if self.audit_mode:
                    # Only log in audit mode
                    logger.warning(
                        "PHI detected in response from %s (audit mode)",
                        path
                    )
                    ...
                    logger.info(
                        "Sanitized PHI in response from %s",
                        path
                    )
            ...
            logger.error(
                "Error sanitizing response from %s: %s",
                path,
                str(e)
            )
