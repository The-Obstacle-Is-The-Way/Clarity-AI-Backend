...
from app.infrastructure.security.jwt_service import JWTService, TokenPayload, TokenType
...
class TestJWTService:
    ...
    async def test_create_access_token_structure(self, jwt_service: JWTService):
        ...
        for field in phi_fields:
            assert field not in payload_dict, f"Unexpected PHI field '{field}' found in token payload: {payload_dict}"
    ...
    async def test_create_access_token_expiration(self, jwt_service: JWTService):
        ...
        assert abs(actual_lifetime_seconds - expected_lifetime_seconds) <= 1, \
