...
class TestAuthentication:
    ...
    @pytest.mark.asyncio
    async def test_expired_token(self, client_app_tuple_func_scoped: tuple[AsyncClient, FastAPI], mock_jwt_service: JWTService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_tampered_token(self, client_app_tuple_func_scoped: tuple[AsyncClient, FastAPI], mock_jwt_service: JWTService) -> None:
    ...
    async def test_valid_token_access(
        ...
        mock_user_repo_for_auth.get_user_by_id = mock_get_user_by_id
...
class TestAuthorization:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (UserRole.CLINICIAN, status.HTTP_403_FORBIDDEN),
    ...
    async def test_role_specific_endpoint_access(
        ...
        async def mock_get_user_by_id_inner(*, user_id: uuid.UUID): # Renamed inner parameter
            if user_id == user_id_param: # Compare with the correct user_id_param from outer scope
                return User(id=user_id_param, username=token_data["username"], email="user@example.com", full_name=f"{token_data['username']} Test Full Name", roles=[user_role], status=UserStatus.ACTIVE, password_hash="hash")
            return None
    ...
    async def mock_get_user_for_unknown_role(*, user_id: uuid.UUID) -> User | None:
        if str(user_id) == unknown_role_user_sub:
            return User(
                ...
                roles=set(unknown_role_user_data["roles"]), # This will be a set with the string
                status=UserStatus.ACTIVE
            )
        return None
