...
class TestAPIHIPAACompliance:
        ...
        with patch("app.config.settings.get_settings", return_value=MockSettings()) as mock_get_settings:
            ...
            async def get_patient(patient_id: str, request: Request):
                ...
                token_map = {
                    ...
                    "valid-doctor-token": {"sub": "doctor-user-id", "roles": ["doctor"], "patient_ids": ["P67890"]},
                    "valid-patient-token": {"sub": "P12345", "roles": ["patient"], "patient_ids": ["P12345"]},
                    "valid-other-patient-token": {"sub": "P_OTHER", "roles": ["patient"], "patient_ids": ["P_OTHER"]}
                }
                ...
                if patient_id == "P12345":
                    return JSONResponse(status_code=200, content={"id": "P12345", "name": "Test Patient"})
                elif patient_id == "P67890":
                    return JSONResponse(status_code=200, content={"id": "P67890", "name": "Doctor's Patient"})
                elif patient_id == "P_OTHER":
                    return JSONResponse(status_code=200, content={"id": "P_OTHER", "name": "Other Patient"})
            ...
            async def create_patient(request: Request):
                ...
                try:
                    ...
                    response_data = {
                        ...
                        "date_of_birth": patient_data.get("date_of_birth", datetime.now().date().isoformat()),
    ...
    def test_patient_data_isolation(self, client, api_prefix, patient_token, other_patient_token):
        ...
        response = client.get(
            f"{api_prefix}/patients/P_OTHER",
            headers={"Authorization": patient_token}
        )
        assert response.status_code == 403
    
    def test_proper_authentication_and_authorization(self, client, api_prefix, admin_token, doctor_token):
    ...
    def test_phi_sanitization_in_response(self, client, api_prefix, admin_token):
        ...
        response = client.get(
            f"{api_prefix}/patients/P12345_phi",
            headers={"Authorization": admin_token}
        )
        assert response.status_code == 200
        data = response.json()
