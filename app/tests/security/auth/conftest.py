...
from app.domain.exceptions.auth_exceptions import InvalidCredentialsException, AccountDisabledException, InvalidTokenException
...
def auth_service_patch(mock_auth_service) -> Generator:
    ...
    """
    with patch("app.presentation.api.v1.endpoints.auth.get_auth_service", return_value=mock_auth_service):
        yield
...
def jwt_service_patch(test_settings) -> Generator:
    ...
    with patch("app.infrastructure.security.jwt.jwt_service.JWTService.create_access_token") as mock_create_access_token, \
         patch("app.infrastructure.security.jwt.jwt_service.JWTService.create_refresh_token") as mock_create_refresh_token:
    ...
    async def patched_dispatch(self, request, call_next):
        ...
        if path.endswith('/api/v1/auth/session-info'):
            ...
            request.state.token_payload = {
                ...
                "roles": [role.value if hasattr(role, "value") else str(role) for role in authenticated_user.roles]
            }
        ...
        if any(public_path in path for public_path in [
            "/auth/login", "/docs", "/openapi.json", "/_debug",
...
@pytest.fixture
async def app_instance(test_settings, auth_service_patch, jwt_service_patch, mock_auth_service, middleware_patch, authenticated_user) -> FastAPI:
    """
...
@pytest.fixture
async def client_app_tuple_func_scoped(app_instance) -> AsyncGenerator[Tuple[AsyncClient, FastAPI], None]:
    """
...
def get_valid_auth_headers() -> Dict[str, str]:
    ...
    """
    return {
        "Authorization": "Bearer test_access_token_for_patient"
    }
...
def get_valid_provider_auth_headers() -> Dict[str, str]:
    ...
    """
    return {
        "Authorization": "Bearer test_access_token_for_provider" 
    } 