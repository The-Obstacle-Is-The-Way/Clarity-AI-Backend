...
class TestSQLAlchemyUnitOfWork:
    ...
    def test_transaction_rollback_without_commit(
        self, unit_of_work, mock_session_factory
    ):
    ...
    def test_nested_transaction_support_commit(
        self, unit_of_work, mock_session_factory
    ):
    ...
    def test_read_only_transaction_prevents_commits(
        ...
        with pytest.raises(
            RepositoryError, match="Cannot commit changes in a read-only transaction"
        ):
    ...
    def test_nested_read_only_transaction(self, unit_of_work, mock_session_factory):
        ...
        with unit_of_work as uow_outer:
            with uow_outer.read_only() as uow_ro_inner:
                ...
                with pytest.raises(
                    RepositoryError,
                    match="Cannot commit changes in a read-only transaction",
                ):
                    uow_ro_inner.commit()
    ...
    def test_transaction_metadata_for_audit(
        ...
        with patch(
            "app.infrastructure.persistence.sqlalchemy.unit_of_work.unit_of_work.get_audit_logger"
        ) as mock_get_logger:
    ...
    def test_user_context_tracking(self, unit_of_work, mock_session_factory):
        ...
        with unit_of_work as uow:
            # Set user context
            uow.set_user_context(user_id="doctor_smith", access_reason="treatment")
