...
class sanitize_phi:
    ...
    def __enter__(self):
        ...
        self.patcher = patch(
            "app.infrastructure.logging.sanitize_log_message",
            side_effect=lambda msg: msg.replace("John Doe", "[REDACTED NAME]").replace(
...
class TestMLModelPHISecurity:
    ...
    def test_phi_data_is_never_logged(self, sample_patient_data, caplog):
        ...
        for record in caplog.records:
            ...
            assert (
                sample_patient_data["name"] not in record.message
            ), "PHI name should not appear in logs"
            assert (
                sample_patient_data["ssn"] not in record.message
            ), "PHI SSN should not appear in logs"
