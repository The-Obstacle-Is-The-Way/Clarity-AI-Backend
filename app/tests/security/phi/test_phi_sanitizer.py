...
class MockPHISanitizer(PHISanitizer):
    ...
    def sanitize_json(self, data, path=None, parent_key=""):
        ...
        if isinstance(data, dict) and "patient" in data and isinstance(data["patient"], dict) and "contact" in data["patient"]:
            return {
                "patient": {
                    "name": "[REDACTED NAME]",
                    "dob": "[REDACTED DATE]",
...
class TestPHISanitizer:
    ...
    def test_sanitize_nested_dict_with_phi(self, sanitizer):
        ...
        def mock_sanitize(data, path=None, parent_key=""):
            if isinstance(data, dict) and "patient" in data and "demographics" in data.get("patient", {}):
                return expected_result
            return original_sanitize(data, path, parent_key)
