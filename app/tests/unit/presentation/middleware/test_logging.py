...
@patch("time.time")
def test_logging_middleware_success(mock_time: MagicMock, mock_logger: MagicMock) -> None:
    ...
    routes = [
        Route("/test/success", endpoint=success_endpoint)
    ]
    ...
    generated_request_id = response.headers["x-request-id"] # Get ID generated by middleware
...
@patch("time.time")
def test_logging_middleware_exception(mock_time: MagicMock, mock_logger: MagicMock) -> None:
    """Test exception scenario logging via TestClient middleware stack."""
    start_time = 1678886400.0
    exception_time = 1678886401.5 # time.time() called at start and before logging exception
    ...
    routes = [
        Route("/test/exception", endpoint=exception_endpoint)
    ]
