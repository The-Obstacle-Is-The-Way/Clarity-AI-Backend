...
def mock_digital_twin_service():
    ...
    service.analyze_clinical_text_mentallama = (
        AsyncMock()
    )  # Assuming this method exists for the endpoint
...
async def client(
    app: FastAPI,
) -> AsyncClient:  # Changed to async fixture and AsyncClient
...
def sample_status_response(sample_patient_id):
    ...
    return {
        ...
        "completeness": 60,
        "components": {
            "symptom_forecasting": {"has_model": True, "last_updated": now_iso},
            "biometric_correlation": {"has_model": True, "last_updated": now_iso},
            "pharmacogenomics": {
...
def sample_clinical_text_analysis_response():
    ...
    return {
        ...
        "metadata": {"processing_time_ms": 254, "model_version": "mentallama-v2.1"},
    }
...
class TestDigitalTwinsEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_get_twin_status(
        self,
        client,
        ...
        response = await client.get(
            f"/api/v1/digital-twins/digital-twin/{sample_patient_id}/status"
        )
    ...
    async def test_get_comprehensive_insights(
        ...
        mock_digital_twin_service.generate_comprehensive_patient_insights.return_value = (
            insight_response
        )
    ...
    @pytest.mark.asyncio
    async def test_analyze_clinical_text(
        self,
        client,
        mock_digital_twin_service,
        sample_patient_id,
    ...
    async def test_analyze_clinical_text_service_error(
        ...
        # Configure the mock to raise our custom exception
        mock_digital_twin_service.analyze_clinical_text_mentallama.side_effect = (
            ModelExecutionError("MentaLLaMA service error")
        )
