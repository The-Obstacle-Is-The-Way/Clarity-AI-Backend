...
def sample_domain_patient_data() -> dict:
    return {
        ...
        "preferences_restrictions": {
            "diet": "Vegetarian",
            "activity": "No high-impact exercise",
        },  # Add preferences
        "custom_fields": {
        ...
        "emergency_contacts": [  # Add emergency contacts
            {"name": "Jane Doe", "relationship": "Spouse", "phone": "555-0303"}
        ],
        "gender": Gender.MALE,
        "address": {
...
class TestPatientModelEncryptionAndTypes:
    ...
    @pytest.mark.asyncio
    @patch(
        "app.infrastructure.persistence.sqlalchemy.types.encrypted_types.global_encryption_service_instance"
    )
    ...
    async def test_encrypted_json_process_result_value(
        ...
        # Test data
        original_python_object = {
            "key": "value",
            "list": [1, 2, {"sub_key": "sub_val"}],
        }
    ...
    async def test_patient_model_to_domain(
        ...
        """
        logger.info(
            f"Starting test_patient_model_to_domain with sample data: {sample_domain_patient_data}"
        )
        ...
        model_instance._medical_history = (
            f"encrypted_{json.dumps(sample_domain_patient_data['medical_history'])}"
        )
        ...
        assert (
            domain_entity.medical_record_number_lve
