...
class TestRedisTokenBlacklistRepository:
    ...
    @pytest.mark.asyncio
    async def test_add_to_blacklist(self, token_blacklist_repo, mock_redis_service, mock_token, mock_jti, future_expiry):
    ...
    @pytest.mark.asyncio
    async def test_add_expired_token_to_blacklist(self, token_blacklist_repo, mock_redis_service, mock_token, mock_jti, past_expiry):
    ...
    @pytest.mark.asyncio
    async def test_is_blacklisted_not_found(self, token_blacklist_repo, mock_redis_service, mock_token):
    ...
    async def test_blacklist_session(self, token_blacklist_repo, mock_redis_service):
        ...
        assert any(arg > 2500000 for arg in [arg for args in mock_redis_service.set.call_args[1].values() for arg in ([args] if not isinstance(args, (list, tuple)) else args)])
    ...
    @pytest.mark.asyncio
    async def test_redis_exception_handling(self, token_blacklist_repo, mock_redis_service, mock_token):
