...
class TestTreatmentResponseModel:
    ...
    def model(self):
        ...
        with patch(
            ...
            model._side_effect_model.predict = MagicMock(
                return_value=np.array([1, 1, 0])
            )  # Example return
            model._side_effect_model.predict_proba = MagicMock(
    ...
    async def test_initialize_loads_model_and_medication_data(
        ...
        mock_json_patch = patch(
            "app.infrastructure.ml.pharmacogenomics.treatment_model.json", autospec=True
        )
        mock_open_patch = patch(
            "app.infrastructure.ml.pharmacogenomics.treatment_model.open",
            new_callable=MagicMock,
        )  # Use MagicMock for open
    ...
    async def test_initialize_handles_missing_files(
        ...
        try:
            ...
            model = TreatmentResponseModel(  # Corrected indentation
                model_path="nonexistent_path", medication_data_path="nonexistent_path"
            )

            # Execute and assert
            with pytest.raises(
                FileNotFoundError,
                match="Model file not found at nonexistent_path or medication data file not found at nonexistent_path",
            ):
                await model.initialize()
    ...
    async def test_predict_treatment_response_success(  # Added colon
        ...
        metabolizer_status = {"CYP2D6": "normal", "CYP2C19": "intermediate"}
        ...
        assert (
            "lowest_side_effects" in result["comparative_analysis"]
        )  # Added colon if this was meant to be a function def start
    ...
    async def test_format_side_effects_result(self, model):  # Added colon, corrected indentation
        ...
        # Execute # Corrected indentation
        result = model._format_side_effects_result(medication, side_effect_risks)
