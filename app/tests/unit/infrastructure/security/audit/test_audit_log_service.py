...
class TestAuditLogMiddleware:
    ...
    def _prepare_request_mock(
        ...
        if user_id:
            ...
            user_details_mock = MagicMock(
                id=user_id
            )  # Ensure 'id' attribute holds the direct string value
    ...
    async def test_dispatch_phi_path_logs_when_enabled(
        self,
        ...
        with patch.object(
            middleware, "_extract_resource_info", return_value=("patients", "123")
        ) as mock_extract_resource:
    ...
    @patch.object(AuditLogMiddleware, "_is_audit_disabled", new_callable=AsyncMock)
    async def test_audit_disabled_bypasses_logging_for_phi_path(
        self,
        mock_is_disabled_method: AsyncMock,
        middleware: AuditLogMiddleware,
        mock_audit_logger: MagicMock,
    ...
    @patch.object(AuditLogMiddleware, "_is_audit_disabled", new_callable=AsyncMock)
    async def test_path_in_skip_list_is_skipped_early(
        self,
        mock_is_disabled_method: AsyncMock,
        middleware: AuditLogMiddleware,
        mock_audit_logger: MagicMock,
    ...
    async def test_logging_failure_does_not_break_request_flow(
        ...
        request = self._prepare_request_mock(
            path="/api/v1/patients/logfailure-case/phi", method="PUT"
        )
        call_next_response = Response(status_code=202, content="Accepted")
        call_next = AsyncMock(return_value=call_next_response)

        # Simulate the logger itself failing
        mock_audit_logger.log_phi_access.side_effect = asyncio.TimeoutError(
            "Simulated logging database timeout"
        )

        with patch.object(
            middleware,
            "_extract_resource_info",
            return_value=("patients", "logfailure-case"),
        ):
            response = await middleware.dispatch(request, call_next)

        assert (
            response == call_next_response
        )  # Original request should succeed despite logging error
        call_next.assert_called_once_with(request)
    ...
    async def test_extract_user_id_when_no_current_user(self, middleware: AuditLogMiddleware):
        ...
        )  # user_id=None sets request.state.user to None
        extracted_user_id = await middleware._extract_user_id(request)
    ...
    async def test_extract_user_id_when_current_user_has_no_id(
        ...
        extracted_user_id = await middleware._extract_user_id(request)
        assert (
            extracted_user_id is None
        )  # getattr on user_without_id_mock for 'id' will return None (or raise if spec strict)
    ...
    def test_extract_resource_info_various_paths(self, middleware: AuditLogMiddleware):
        ...
        test_cases = [
            ...
            (
                "/api/v1/medical-records/record-abc-456",
                ("medical-records", "record-abc-456"),
            ),
            ("/api/v1/users/user789", ("users", "user789")),
            (
                "/api/v1/devices/dev_id_with_underscore/data",
                ("devices", "dev_id_with_underscore"),
            ),
            ("/api/v1/no_id_path", ("no_id_path", None)),
            ("/api/v1/patients/", ("patients", None)),  # Common for POST to collection
            (
                "/api/v1/consultations/consult-guid-goes-here/notes/note-sub-id",
                ("consultations", "consult-guid-goes-here"),
            ),
        ]
