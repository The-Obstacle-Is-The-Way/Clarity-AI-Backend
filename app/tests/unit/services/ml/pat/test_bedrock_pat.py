...
class MockS3Service(S3ServiceInterface):
    ...
    def list_objects(
        ...
        return {"Contents": contents, "Name": bucket_name, "KeyCount": len(contents)}
...
class MockComprehendMedicalService(ComprehendMedicalServiceInterface):
    ...
    def infer_icd10_cm(self, text: str) -> dict[str, Any]:
        return {"Entities": [], "ModelVersion": "MockVersion"}
...
class MockBedrockRuntimeService(BedrockRuntimeServiceInterface):
    ...
    def invoke_model(
        self, model_id: str, body, content_type=None, accept=None, **kwargs
    ) -> dict[str, Any]:
        return {
            "body": BodyWrapper(
                json.dumps(
...
async def test_analyze_actigraphy(bedrock_pat_service, pat_config):
    ...
    with patch.object(
        bedrock_pat_service, "analyze_actigraphy", new_callable=AsyncMock
    ) as mock_analyze:
...
async def test_get_actigraphy_embeddings(bedrock_pat_service):
    ...
    result = await bedrock_pat_service.get_actigraphy_embeddings(
        patient_id=patient_id, readings=readings
    )
...
async def test_get_analysis_by_id(bedrock_pat_service):
    ...
    # Use AsyncMock for async methods
    with patch.object(
        bedrock_pat_service, "get_analysis_by_id", new_callable=AsyncMock
    ) as mock_get:
        # Configure the mock to return our defined value
...
async def test_get_patient_analyses(bedrock_pat_service):
    ...
    # Use AsyncMock for async methods
    with patch.object(
        bedrock_pat_service, "get_patient_analyses", new_callable=AsyncMock
    ) as mock_get_patient:
