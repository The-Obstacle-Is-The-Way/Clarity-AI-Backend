...
class TestDataProcessingBenchmarks:
    ...
    def test_json_deserialization_benchmark(self, benchmark) -> None:
        ...
        complex_data = {"items": [{"id": i, "value": f"item_{i}"} for i in range(1000)]}
...
class TestMockDatabaseBenchmarks:
    ...
    def test_mock_database_query_benchmark(self, benchmark) -> None:
        ...
        # Mock database data
        mock_users = [{"id": i, "name": f"User {i}", "active": i % 2 == 0} for i in range(1000)]
    ...
    def test_mock_database_insert_benchmark(self, benchmark) -> None:
        ...
        def insert_batch():
            nonlocal mock_db
            batch = [{"id": i, "timestamp": datetime.now().isoformat()} for i in range(100)]
        ...
        def complex_query():
            ...
            for user in users:
                if user["id"] % 5 == 0:  # Filter condition
                    ...
                    result.append({"user_id": user["id"], "department_name": dept["name"]})
            return result
