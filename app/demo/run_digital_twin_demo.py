...
async def run_demo():
    ...
    print(
        f"Initial metrics: PHQ-9 Score: {initial_state.clinical_metrics.get('phq9_score')}, "
    ...
    recommendations = await digital_twin_core.generate_treatment_recommendations(
        patient_id=saved_patient.id, include_rationale=True
    )
    ...
    for idx, rec in enumerate(recommendations[:3]):  # Show top 3
        ...
        print(
            f"{idx+1}. {rec['type'].title()}: {rec['name']} "
