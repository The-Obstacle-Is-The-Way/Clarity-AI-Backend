...
def audit_phi_access(
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            access_reason = kwargs.pop(
                "audit_reason", get_current_access_reason() or default_reason
            )
            ...
            if not user_id:
                logger.warning(
                    f"PHI access without user ID: {resource_type}:{resource_id} {action}"
                )
...
def audit_async_phi_access(
    ...
    def decorator(func: AsyncF) -> AsyncF:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if not user_id:
                logger.warning(
                    f"Async PHI access without user ID: {resource_type}:{resource_id} {action}"
                )
...
def export_audit_logs(
    ...
    return audit_logger.export_logs(
        start_date=start_date, end_date=end_date, format=format, verify_integrity=True
    )
