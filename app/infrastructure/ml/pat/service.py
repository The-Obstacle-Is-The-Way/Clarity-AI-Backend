...
class PATService:
    ...
    async def analyze(
        ...
        try:
            ...
            results = await self._process_predictions(
                raw_predictions, analysis_type, patient_metadata
            )
    ...
    async def _process_predictions(
        ...
        if analysis_type == AnalysisType.SLEEP_QUALITY:
            ...
            results = await self._process_medication_response(
                predictions, results, patient_metadata
            )
    ...
    async def _process_sleep_quality(
        ...
        if sleep_efficiency < 0.7:
            results["insights"].append(
                "Poor sleep efficiency detected. Consider sleep hygiene interventions."
            )
        elif sleep_efficiency < 0.8:
            results["insights"].append(
                "Moderate sleep efficiency. Monitor for changes in sleep patterns."
            )
        else:
    ...
    async def _process_activity_patterns(
        ...
        if daily_steps < 5000:
            results["insights"].append(
                "Low physical activity detected. Consider gradual increase in daily movement."
            )
        elif daily_steps < 7500:
            results["insights"].append(
                "Moderate physical activity. Encourage maintaining or increasing activity levels."
            )
        else:
    ...
    async def _process_circadian_rhythm(
        ...
        if rhythm_stability < 0.6:
            results["insights"].append(
                "Irregular circadian rhythm detected. Consider regular sleep-wake schedule."
            )
        elif rhythm_stability < 0.7:
            results["insights"].append(
                "Moderately stable circadian rhythm. Minor improvements in sleep schedule consistency recommended."
            )
        else:
    ...
    async def _process_energy_expenditure(
        ...
        if (
            patient_metadata
            and "height_cm" in patient_metadata
            and "weight_kg" in patient_metadata
            and "age" in patient_metadata
            and "gender" in patient_metadata
        ):
        ...
        if activity_level < 1.4:
            results["insights"].append(
                "Sedentary activity level detected. Consider increasing daily activity."
            )
        elif activity_level < 1.6:
            results["insights"].append(
                "Light activity level. Gradual increase in physical activity recommended."
            )
        else:
    ...
    async def _process_mental_state_correlation(
        ...
        if depression_risk > 0.6:
            results["insights"].append(
                "Activity patterns suggest elevated depression risk. Clinical assessment recommended."
            )
            results["warnings"].append("Elevated depression risk indicators detected.")
        elif depression_risk > 0.4:
            results["insights"].append(
                "Activity patterns suggest moderate depression risk. Monitor for changes."
            )

        if anxiety_risk > 0.6:
            results["insights"].append(
                "Activity patterns suggest elevated anxiety risk. Clinical assessment recommended."
            )
            results["warnings"].append("Elevated anxiety risk indicators detected.")
        elif anxiety_risk > 0.4:
            results["insights"].append(
                "Activity patterns suggest moderate anxiety risk. Monitor for changes."
            )
    ...
    async def _process_medication_response(
        ...
        if (
            patient_metadata
            and "medications" in patient_metadata
            and patient_metadata["medications"]
        ):
    ...
    async def get_model_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "parameters": {"small": "~10M", "medium": "~50M", "large": "~100M"}[
