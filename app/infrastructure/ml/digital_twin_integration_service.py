...
class DigitalTwinIntegrationService:
    ...
    async def get_digital_twin_by_patient(self, patient_id: str) -> Optional[DigitalTwin]:
        """
                    ...
                    if abs(trend) > 0.3:
                        ...
                        recommendations.append({
                            ...
                            "recommendation": f"{symptom.title()} symptoms show {direction} trend. " + 
                                             ("Continue current treatment plan." if trend > 0 else "Consider treatment adjustment."),
                ...
                if "efficacy" in med_data and med_data["efficacy"].get("score", 0) > 0.7:
                    ...
                    recommendations.append({
                        ...
                        "recommendation": f"{med_name.title()} shows high predicted efficacy" + 
                                         (f" with {len(side_effects_text)} potential side effects." if side_effects_text else "."),
                        "confidence": confidence,
                        "supporting_evidence": [
                            f"Efficacy score: {efficacy_score:.2f}",
                            f"Confidence: {confidence:.2f}"
                        ] + (["Side effects: " + ", ".join(side_effects_text)] if side_effects_text else []),
                        "priority": "medium" 
                    })
