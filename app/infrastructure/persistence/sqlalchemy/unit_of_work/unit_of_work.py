...
class SQLAlchemyUnitOfWork(UnitOfWork):
    ...
    def __init__(self, session_factory: sessionmaker):
        ...
        if not isinstance(session_factory, sessionmaker):
            ...
            if not callable(session_factory):
                raise TypeError(
                    "session_factory must be a SQLAlchemy sessionmaker instance or a callable factory."
                )
        ...
        try:
            if exc_type:
                ...
                if is_read_only:
                    ...
                    if is_top_level:
                        logger.warning(
                            f"Rolling back {log_level} transaction: No explicit commit was called."
                        )
                        ...
                        logger.debug(
                            f"Rolling back {log_level} transaction (savepoint): No explicit commit."
                        )
    ...
    def read_only(self) -> ContextManager["SQLAlchemyUnitOfWork"]:
        ...
        if (
            ...
            and self._is_read_only_stack
            and self._is_read_only_stack[-1]
        ):
        ...
        with self as uow:  # Calls __enter__, increments level, pushes False (or inherited) to stack
            ...
            if uow._is_read_only_stack:  # Should exist after __enter__
                ...
                logger.debug(
                    f"Entered read-only transaction context (level {uow._transaction_level})."
                )
                try:
                    yield uow
                finally:
