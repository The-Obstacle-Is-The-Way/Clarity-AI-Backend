...
class SQLAlchemyBiometricRuleRepository(IBiometricRuleRepository):
    ...
    async def add(self, rule: BiometricAlertRule) -> BiometricAlertRule:
        ...
        try:
            ...
            raise DatabaseConnectionException(
                f"Database error adding rule: {e!s}"
            ) from e
    ...
    async def get_by_id(self, rule_id: UUID) -> BiometricAlertRule | None:
        ...
        try:
            ...
            raise RepositoryError(
                f"Unexpected error retrieving rule {rule_id}: {e!s}"
            ) from e
    ...
    async def get_all(self) -> list[BiometricAlertRule]:
        ...
        try:
            ...
            raise DatabaseConnectionException(
                f"Database error retrieving all rules: {e!s}"
            ) from e
        except Exception as e:
            logger.error(f"Unexpected error retrieving all rules: {e}")
            raise RepositoryError(
                f"Unexpected error retrieving all rules: {e!s}"
            ) from e
        ...
        try:
            ...
            if not existing_rule:
                raise EntityNotFoundException(
                    f"Biometric rule with ID {rule.id} not found"
                )
            rule_model = map_rule_entity_to_model(rule)
            stmt = (
            ...
            raise DatabaseConnectionException(
                f"Database error updating rule: {e!s}"
            ) from e
        except Exception as e:
            await self.session.rollback()
    ...
    async def delete(self, rule_id: UUID) -> bool:
        ...
        try:
            ...
            if not existing_rule:
                logger.warning(
                    f"Attempted to delete non-existent rule with ID: {rule_id}"
                )
                return False
            ...
            raise RepositoryError(
                f"Unexpected error deleting rule {rule_id}: {e!s}"
            ) from e
    ...
    async def get_by_patient_id(self, patient_id: UUID) -> list[BiometricAlertRule]:
        ...
        try:
            ...
            logger.error(
                f"Database error retrieving rules for patient {patient_id}: {e}"
            )
            raise DatabaseConnectionException(
            ...
            logger.error(
                f"Unexpected error retrieving rules for patient {patient_id}: {e}"
            )
            raise RepositoryError(
    ...
    async def get_by_provider_id(self, provider_id: UUID) -> list[BiometricAlertRule]:
        ...
        try:
            ...
            logger.error(
                f"Database error retrieving rules for provider {provider_id}: {e}"
            )
            raise DatabaseConnectionException(
            ...
            logger.error(
                f"Unexpected error retrieving rules for provider {provider_id}: {e}"
            )
            raise RepositoryError(
    ...
    async def get_all_active(self) -> list[BiometricAlertRule]:
        ...
        try:
            ...
            raise RepositoryError(
                f"Unexpected error retrieving all active rules: {e!s}"
            ) from e
    ...
    async def get_active_rules_for_patient(
        ...
        try:
            ...
            logger.error(
                f"Database error retrieving active rules for patient {patient_id}: {e}"
            )
            raise DatabaseConnectionException(
            ...
            logger.error(
                f"Unexpected error retrieving active rules for patient {patient_id}: {e}"
            )
            raise RepositoryError(
    ...
    async def save(self, rule: BiometricAlertRule) -> BiometricAlertRule:
        ...
        try:
            ...
            logger.warning(
                f"Rule with ID {rule.id} not found for update, attempting to add."
            )
            rule.id = None
            return await self.add(rule)
    ...
    async def count_active_rules(self, patient_id: UUID) -> int:
        ...
        try:
            ...
            logger.error(
                f"Database error counting active rules for patient {patient_id}: {e}"
            )
            raise DatabaseConnectionException(
            ...
            logger.error(
                f"Unexpected error counting active rules for patient {patient_id}: {e}"
            )
            raise RepositoryError(
    ...
    async def update_active_status(self, rule_id: UUID, is_active: bool) -> bool:
        ...
        try:
            ...
            if updated_count == 0:
                ...
                return False
            logger.info(
                f"Successfully updated active status for rule {rule_id} to {is_active}"
            )
            return True
        except SQLAlchemyError as e:
            await self.session.rollback()
            logger.error(
                f"Database error updating active status for rule {rule_id}: {e}"
            )
            raise DatabaseConnectionException(
            ...
            await self.session.rollback()
            logger.error(
                f"Unexpected error updating active status for rule {rule_id}: {e}"
            )
            raise RepositoryError(
