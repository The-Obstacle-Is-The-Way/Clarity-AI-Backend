            ...
            if existing_model:
                ...
                try:
                    ...
                    raise RepositoryError(
                        f"Error mapping alert entity to model: {mapping_err}"
                    ) from mapping_err
    ...
    async def get_by_id(self, alert_id: UUID | str) -> BiometricAlert | None:
        ...
        try:
            ...
            try:
                ...
                raise RepositoryError(
                    f"Error querying alert with ID {alert_id}: {query_err}"
                ) from query_err
    ...
    async def get_by_patient_id(
        ...
        try:
            ...
            try:
                ...
                raise RepositoryError(
                    f"Error querying alerts for patient {patient_id}: {query_err}"
                ) from query_err
    ...
    def _map_to_entity(self, model: BiometricAlertModel) -> BiometricAlert:
        ...
        if hasattr(model, "_extract_mock_name") and hasattr(model, "__class__"):
            ...
            try:
                ...
                raise RepositoryError(
                    f"Failed to map mock model to domain entity: {mock_err}"
                ) from mock_err
    ...
    def _update_model(self, model: BiometricAlertModel, entity: BiometricAlert) -> None:
        ...
        try:
            ...
            if entity.acknowledged_by is not None:
                model.acknowledged_by = (
                    str(entity.acknowledged_by) if entity.acknowledged_by else None
                )
            ...
            raise RepositoryError(
                f"Failed to update database model from domain entity: {e!s}"
            ) from e
