...
class PatientRepository:
    ...
    async def _with_session(self, operation):
        ...
        if self.uow_session is not None:
            ...
            raise RuntimeError(
                "No database session or factory available"
            )  # Changed from RepositoryException for clarity

    async def create(
        self, patient_entity: PatientEntity, context: dict | None = None
    ) -> PatientEntity | None:
        """Creates a new patient record in the database from a PatientEntity."""
        self.logger.debug(
            f"Attempting to create patient with entity ID: {patient_entity.id} with context: {context}"
        )
        ...
        async def _create_operation(session: AsyncSession) -> PatientEntity | None:
            try:
                ...
                patient_model = await PatientModel.from_domain(
                    patient_entity
                ...
                ValidationError
            ) as e:  # This is pydantic.ValidationError (should be PydanticV2ValidationError)
                ...
                raise PersistenceError(
                    f"Pydantic V2 Validation Error: {e.errors()}", original_exception=e
                )
    ...
    async def get_by_id(
        self, patient_id: str | UUID, context: dict | None = None
    ) -> PatientEntity | None:
        ...
        async def _get_by_id_operation(session):
            ...
            if patient_model:
                self.logger.debug(
                    f"Patient model found for ID {patient_uuid}. Converting to domain entity."
                )
    ...
    async def get_all(
        self, limit: int = 50, offset: int = 0, context: dict | None = None
    ) -> list[PatientEntity]:
    ...
    async def update(
        ...
        if not domain_dict and hasattr(patient_entity, "__dict__"):
            try:
                domain_dict = {
                    k: v
                    for k, v in patient_entity.__dict__.items()
        ...
        for key, value in domain_dict.items():
            if key not in field_map and not key.endswith(
                "_lve"
            ):  # Avoid re-adding LVEs or mapped fields
                if not hasattr(PatientModel, key) and hasattr(
                    PatientModel, f"_{key}"
                ):  # check if it's a private version
        ...
        if not update_data_for_model:
            self.logger.warning(
                f"No updatable fields found for patient ID: {patient_id} from entity: {patient_entity}"
            )
            ...
            try:
                ...
                db_patient = await self._with_session(
                    lambda session: session.get(PatientModel, db_id)
                )
                if db_patient:
        ...
        async def _update_operation(session: AsyncSession) -> PatientEntity | None:
            self.logger.debug(
                f"Executing update for patient ID: {patient_id} with model data: {update_data_for_model}"
            )
            ...
            if db_patient:
                ...
                for key, value in update_data_for_model.items():  # Use the mapped data
                    if hasattr(db_patient, key):
                        ...
                        self.logger.warning(
                            f"Attribute {key} not found on PatientModel during update for patient ID: {patient_id}"
                        )

                if not updated_fields_for_log:
                    self.logger.info(
                        f"No fields were actually updated for patient ID: {patient_id} based on provided data."
                    )
                        ...
                        if "Mock" in db_patient.to_domain.__class__.__name__:
                            ...
                            try:
                                ...
                                return PatientEntity(
                                    ...
                                    or "DefaultFirstName",
                                    last_name=getattr(db_patient, "_last_name", None)
                                    or "DefaultLastName",
                ...
                self.logger.info(
                    f"Successfully updated patient data for DB ID: {db_patient.id}. Context: {context}"
                )
    ...
    async def get_by_email(self, email: str) -> PatientEntity | None:
        ...
        async def _get_by_email_operation(
            ...
            try:
                ...
                self.logger.error(
                    f"Database error retrieving patient by email {email}: {e}",
                    exc_info=True,
                )
                ...
                self.logger.error(
                    f"Unexpected error retrieving patient by email {email}: {e}",
                    exc_info=True,
                )
                raise PersistenceError(
