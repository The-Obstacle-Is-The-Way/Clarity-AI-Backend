        ...
        if hasattr(model, "roles") and model.roles:
            for role_in_model in model.roles:
                try:
                    ...
                    role_value = (
                        role_in_model.value
                        if hasattr(role_in_model, "value")
                        else str(role_in_model)
                    )
                    ...
                    print(
                        f"Warning: Invalid role value '{role_in_model}' from DB for user {model.id}: {e_role}"
                    )
