...
class BiometricTwinModel(Base, TimestampMixin):
    ...
    timeseries_data = relationship("BiometricTimeseriesDataModel", back_populates="biometric_twin", cascade="all, delete-orphan")
...
class BiometricDataPointModel(Base):
    ...
    twin_id = Column(SQLAlchemyUUID(as_uuid=True), ForeignKey("biometric_twins.id"), index=True, nullable=False)
...
class BiometricTimeseriesDataModel(Base, TimestampMixin):
    ...
    id = Column(SQLAlchemyUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    biometric_twin_id = Column(SQLAlchemyUUID(as_uuid=True), ForeignKey("biometric_twins.id"), nullable=False, index=True)
