...
class AuditLogRepository(IAuditLogRepository):
    ...
    async def get_statistics(
        ...
        ).where(base_condition).group_by(AuditLogModel.event_type)
        ...
        success_query = select(
            ...
            func.count(AuditLogModel.id)
        ).where(base_condition).group_by(AuditLogModel.success)
        ...
        user_query = select(
            ...
            and_(AuditLogModel.user_id != None, base_condition)  # noqa: E711
        ).group_by(AuditLogModel.user_id).order_by(func.count(AuditLogModel.id).desc()).limit(10)
