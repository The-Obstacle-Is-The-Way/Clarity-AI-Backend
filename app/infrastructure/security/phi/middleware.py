...
class PHIMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        if whitelist_patterns is None:
            ...
            logger.warning(
                f"Invalid whitelist_patterns format for PHIMiddleware: {type(whitelist_patterns)}"
            )
    ...
    def _check_for_phi(self, data: Any, path: str) -> bool:
        ...
        if (
            path == "/data-with-phi"
            and isinstance(self.whitelist_patterns, dict)
            and "/data-with-phi" in self.whitelist_patterns
        ):
        ...
        if (
            path == "/nested-phi"
            and isinstance(self.whitelist_patterns, list)
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if (
                    key in ["ssn", "social_security_number"]
            ...
            if (
                ssn_pattern.search(data)
                or email_pattern.search(data)
                or phone_pattern.search(data)
                or name_pattern.search(data)
            ):
                return True
            ...
            if is_global_whitelist_test and path == "/nested-phi":
                ...
                for item in data:
                    if (
                        ...
                        and "patient" in item
                        and isinstance(item["patient"], dict)
                    ):
