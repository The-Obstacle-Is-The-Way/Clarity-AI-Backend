...
class AuditLogMiddleware(BaseHTTPMiddleware):
    ...
    async def _is_audit_disabled(self, request: Request) -> bool:
        ...
        if hasattr(request.app.state, "settings") and hasattr(
        ...
        if (
            hasattr(request.app.state, "disable_audit_middleware")
            and request.app.state.disable_audit_middleware
        ):
            return True

        # Check if disabled on request state
        if (
            hasattr(request.state, "disable_audit_middleware")
            and request.state.disable_audit_middleware
        ):
            return True
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if not await self._is_audit_disabled(
                request
            ):  # Check again in case middleware was disabled during request
                try:
            ...
            if not await self._is_audit_disabled(request):
                try:
                    ...
                    logger.error(
                        f"Error in audit logging during exception handling: {log_error}",
                        exc_info=True,
                    )
    ...
    def _extract_resource_info(self, path: str) -> tuple[str | None, str | None]:
        ...
        if generic_match:
            ...
            if resource_id_match:
                ...
                if (
                    not resource_id
                ):  # If rstrip results in an empty string (e.g. path was /api/v1/type//sub), treat as no ID
                    resource_id = None
            return resource_type, resource_id
