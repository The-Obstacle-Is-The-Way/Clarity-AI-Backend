    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name == "original_claim":
            ...
            if (
                ...
                and self.custom_fields
                and "original_claim" in self.custom_fields
            ):
        ...
        if (
            hasattr(self, "custom_fields")
            and self.custom_fields
            and "custom_fields" in self.custom_fields
            and name in self.custom_fields["custom_fields"]
        ):
        ...
        if (
            hasattr(self, "custom_fields")
            and self.custom_fields
            and "additional_claims" in self.custom_fields
        ):
    ...
    def from_jwt_payload(cls, payload: Any) -> "TokenPayload":
        ...
        if hasattr(payload, "model_dump") and callable(getattr(payload, "model_dump")):
            try:
                ...
                if (
                    data.get("role") is None
                    and hasattr(payload, "role")
        ...
        if isinstance(payload, dict):
            try:
                ...
                logger.warning(
                    f"Extracted data was: {extracted_data if 'extracted_data' in locals() else 'not created'}"
                )
...
class JWTServiceImpl(IJwtService):
    ...
    async def refresh_access_token(self, refresh_token: str) -> str:
        ...
        try:
            ...
            new_access_token = await self.create_access_token_async(
                user_id=refresh_payload.sub, roles=getattr(refresh_payload, "roles", [])
            )
    ...
    async def logout(self, token: str) -> bool:
        ...
        try:
            ...
            payload = self._decode_token(token, verify_exp=False)
            exp_timestamp = payload.exp or int(
    ...
    def create_access_token(
        ...
        **additional_claims: Any,
        ...
        if (
            "additional_claims" in passed_additional_claims
            and passed_additional_claims["additional_claims"]
            and "roles" in passed_additional_claims["additional_claims"]
        ):
    ...
    def _sanitize_message_for_hipaa(self, message: str) -> str:
        ...
        mrn_pattern = (
            r"\b(?:MRN|mrn|medical[_\s]?record[_\s]?number|patient[_\s]?id)[:\s]*[A-Za-z0-9-]{6,}\b"
        )
    ...
    def create_unauthorized_response(
        self, error_type: str = "authentication_required", message: str = "Authentication required"
    ) -> dict[str, Any]:
