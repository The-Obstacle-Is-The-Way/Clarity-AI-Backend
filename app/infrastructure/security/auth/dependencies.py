...
def get_current_active_user(request: Request) -> User:
    ...
    if not isinstance(user, User):
        logger.error(
            f"Dependency error: request.state.user is not a User object (type: {type(user)}). Check AuthenticationMiddleware."
        )
        raise HTTPException(
...
def get_rbac_service_dependency() -> RBACService:
    logger.warning(
        "RBACService is being instantiated directly in dependency. Setup proper DI provider."
    )
    return RBACService()
...
class PermissionsChecker:
    ...
    async def __call__(
        ...
        if not user_roles:
            logger.warning(
                f"User {user.id} has no roles, denying permission check for: {self.required_permissions}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        logger.debug(
            f"Checking permissions {self.required_permissions} for user {user.id} with roles {user_roles}"
        )
        ...
        if missing_permissions:
            logger.warning(
                f"Authorization Denied for User {user.id}: Missing permissions: {missing_permissions}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        logger.debug(
            f"Authorization Granted for User {user.id} for permissions: {self.required_permissions}"
        )
...
class RoleChecker:
    ...
    async def __call__(self, user: User = Depends(get_current_active_user)):
        ...
        if not user_roles:
            logger.warning(
                f"User {user.id} has no roles, denying role check for: {self.required_roles}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="User has no assigned roles.",
        ...
        logger.debug(
            f"Checking roles {self.required_roles} for user {user.id} with roles {user_roles}"
        )

        if not self.required_roles.intersection(user_roles):
            logger.warning(
                f"Authorization Denied for User {user.id}: Missing required roles. Needs one of: {self.required_roles}, Has: {user_roles}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
