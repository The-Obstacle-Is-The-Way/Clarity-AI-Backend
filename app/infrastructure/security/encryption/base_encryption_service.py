...
class BaseEncryptionService:
                    ...
                    if key_lower == "medications":
                        ...
                        for item in value:
                            if isinstance(item, dict):
                                ...
                                result[key].append(
                                    self.encrypt_string(item) if is_sensitive else item
                                )
                    else:
    ...
    def decrypt_file(self, input_path: str, output_path: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Invalid token for decrypting file {input_path}. The file might be corrupted or encrypted with a different key."
            )
    ...
    def generate_hash(self, data: str) -> tuple[str, str]:
        ...
        settings = get_settings()
        iterations = getattr(
            settings, "HASH_ITERATIONS", 390000
