...
class IBiometricTwinRepository(ABC):
    ...
    @abstractmethod
    async def add_data_point(self, patient_id: UUID, data_type: str, value: float | int | bool | dict[str, Any], 
                             timestamp: datetime | None = None) -> BiometricTwinState:
        """Add a new data point to a patient's biometric twin."""
        pass
    
    @abstractmethod
    async def get_latest_data(self, patient_id: UUID, data_type: str | None = None) -> dict[str, Any]:
