...
class MockPATService(PATInterface):
    ...
    def _init_test_profiles(self) -> None:
        ...
        self._profiles[test_profile_id] = {
            ...
            "data": {
                ...
                "sleep_patterns": {"duration": 7.5, "quality": 0.8, "regularity": 0.9},
            },
        }
        ...
        self._profiles[profile456_id] = {
            ...
            "metrics": {
                "activity": {"steps_daily_avg": 8500, "active_minutes_daily": 120},
                "sleep": {"hours_daily_avg": 7.5, "quality_score": 82},
            },
    ...
    def initialize(self, config: dict[str, Any]) -> None:
        ...
        if not config and hasattr(self, "_force_init_error") and self._force_init_error:
            ...
            logger.error(
                "Error initializing service: Simulated error for test_initialization_error"
            )
    ...
    def complete_assessment(
        self, assessment_id: str, completion_data: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    def _create_mock_template(self, form_type: str) -> str:
        ...
        template = {
            ...
            "created_at": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "metadata": {"description": f"Mock template for {form_type} assessment"},
        }
    ...
    def analyze_actigraphy(
        ...
        **kwargs,
        ...
        self._validate_actigraphy_inputs(
            patient_id, readings, sampling_rate_hz, device_info, analysis_types
        )
        ...
        for analysis_type in analysis_types:
            result = self._add_analysis_type_results(
                result, analysis_type, sleep_metrics, activity_levels
            )
        ...
        for i, reading in enumerate(readings):
            ...
            if missing_keys:
                ...
                raise ValidationError(
                    f"Invalid reading format at index {i}: missing required keys {missing_keys_str}."
                )
    ...
    def _validate_analysis_types(self, analysis_types: list[str]) -> None:
        ...
        for at_type in analysis_types:
            if at_type not in VALID_ANALYSIS_TYPES:
                raise ValidationError(
                    f"Invalid analysis type: {at_type}. Valid types are: {VALID_ANALYSIS_TYPES}"
                )
    ...
    def _generate_circadian_rhythm(self) -> dict[str, Any]:
        ...
        return {
            ...
            "melatonin_estimate": {"onset_time": "22:00:00", "peak_time": "02:00:00"},
        }
    ...
    def _generate_behavioral_patterns(self) -> dict[str, Any]:
        ...
        return {
            ...
            "week_patterns": {"weekday_activity": 72.5, "weekend_activity": 58.3},
    ...
    def _add_analysis_type_results(
        ...
        if analysis_type in ["sleep", "sleep_quality", "sleep_analysis"]:
            ...
            result["results"][analysis_type] = {
                ...
                "insights": [
                    "Moderate activity level detected",
                    "Meets daily activity recommendations",
                ],
            }
        else:
    ...
    def _generate_mock_actigraphy_metrics(
        self, readings: list[dict[str, float]], analysis_types: list[str]
    ) -> dict[str, Any]:
    ...
    def integrate_with_digital_twin(
        ...
        **kwargs,  # Added for interface compliance
        ...
        integration_results = self._generate_domain_integration_results(
            integration_types, analysis_data_to_integrate
        )
        ...
        result = {
            "integration_id": integration_id,
            "analysis_id": analysis_data_to_integrate.get("analysis_id"),
            "actigraphy_analysis_id": analysis_data_to_integrate.get(
                "analysis_id"
            ),  # Add this field for test compatibility
            "patient_id": patient_id,
            "profile_id": profile_id,
            "timestamp": timestamp,
            "created_at": timestamp,
            ...
            "integrated_profile": {  # Required by test_integrate_with_digital_twin_success
                ...
                "mood_assessment": {"estimated_mood": "stable", "confidence": 80},
                "insights": {
                    "activity": "moderate activity levels detected",
                    "sleep": "sleep patterns are generally good",
                    "behavioral": "consistent daily patterns observed",
    ...
    def _validate_integration_params(
        ...
        if analysis_id:
            ...
            raise ValidationError(
                "Either analysis_id or actigraphy_analysis must be provided for integration"
            )
    ...
    def get_activity_metrics(
        self, patient_id: str, start_date: str, end_date: str, **kwargs
    ) -> dict[str, Any]:
        """Mock implementation for getting activity metrics."""
        self._check_initialized()
        logger.info(
            f"Mock getting activity metrics for patient {patient_id} from {start_date} to {end_date}"
        )
    ...
    def get_sleep_metrics(
        ...
        self._check_initialized()
        logger.info(
            f"Mock getting sleep metrics for patient {patient_id} from {start_date} to {end_date}"
        )
        return {
            "patient_id": patient_id,
            "start_date": start_date,
