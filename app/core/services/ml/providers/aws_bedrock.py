...
class AWSBedrockMentaLLaMA(BaseMentaLLaMA):
    ...
    def _initialize_provider(self) -> None:
        ...
        try:
            ...
            self._model_mappings = {
                "default": self._get_config_param(
                    "default_model", "anthropic.claude-3-haiku-20240307-v1:0"
                ),
                "depression_detection": self._get_config_param(
                    "depression_detection_model",
                    "anthropic.claude-3-haiku-20240307-v1:0",
                ),
                "risk_assessment": self._get_config_param(
                    "risk_assessment_model", "anthropic.claude-3-opus-20240229-v1:0"
                ),
                "sentiment_analysis": self._get_config_param(
                    "sentiment_analysis_model", "anthropic.claude-3-haiku-20240307-v1:0"
                ),
                "wellness_dimensions": self._get_config_param(
                    "wellness_dimensions_model",
    ...
    def _generate_anthropic(
        self, model: str, prompt: str, max_tokens: int, temperature: float, **kwargs
    ) -> dict[str, Any]:
        """
        ...
        **kwargs,
        ...
        try:
            ...
            full_prompt = self._create_digital_twin_conversation_prompt(
                system_prompt=system_prompt, history=history, user_message=prompt
            )
    ...
    def _create_digital_twin_system_prompt(
        self, patient_id: str, context: dict[str, Any] | None = None
    ) -> str:
        """
