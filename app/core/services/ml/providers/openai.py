...
class OpenAIMentaLLaMA(MentaLLaMAInterface):
    ...
    def initialize(self, config: dict[str, Any]) -> None:
        ...
        try:
            ...
            self._client = None
            raise InvalidConfigurationError(
                f"Failed to initialize OpenAI MentaLLaMA service: {e!s}"
            )
...
Maintain a balanced, strengths-based approach while acknowledging challenges. Emphasize this analysis is meant to support, not replace, clinical judgment and follow all HIPAA guidelines.""",
    ...
    def detect_depression(self, text: str, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        opts[
            "instruction"
        ] = """Analyze the text for potential signals of depression and provide your analysis in the following JSON format:
{
    "depression_signals": {
    ...
    def assess_risk(
        ...
        opts[
            "instruction"
        ] = f"""Analyze the text for potential risk signals. {risk_instruction}Provide your analysis in the following JSON format:
{{
    "risk_assessment": {{
    ...
    def analyze_sentiment(self, text: str, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        opts[
            "instruction"
        ] = """Analyze the emotional content and sentiment of the text. Provide your analysis in the following JSON format:
{
    "sentiment": {
    ...
    def analyze_wellness_dimensions(
        ...
        if dimensions:
            ...
            dimensions_instruction = (
                f"Focus specifically on these wellness dimensions: {dimension_list}. "
            )

        # Add instruction to get structured result
        opts[
            "instruction"
        ] = f"""Analyze the text across different dimensions of wellness. {dimensions_instruction}Provide your analysis in the following JSON format:
{{
    "wellness_dimensions": [
        {{
