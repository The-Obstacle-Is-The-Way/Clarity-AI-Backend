...
class DigitalTwinService(DigitalTwinInterface):
    ...
    def initialize(self, config: dict[str, Any]) -> None:
        ...
        logger.info(
            "Initializing Digital Twin service with config: %s",
            json.dumps(
                {
    ...
    def _update_insights(self, session: dict[str, Any], message: str) -> None:
        ...
        risk_keywords = {"suicide": 3, "hurt": 1, "die": 2, "harm": 2, "plan": 1}
        ...
        for keyword, score in mood_keywords.items():
            if keyword in message.lower():
                ...
                session["insights"]["mood"]["last_updated"] = datetime.datetime.now(
                    timezone.utc
                ).isoformat()
        ...
        for keyword, score in sleep_keywords.items():
            if keyword in message.lower():
                ...
                session["insights"]["sleep"]["last_updated"] = datetime.datetime.now(
                    timezone.utc
                ).isoformat()
        ...
        for keyword, score in risk_keywords.items():
            if keyword in message.lower():
                ...
                session["insights"]["risk"]["last_updated"] = datetime.datetime.now(
                    timezone.utc
                ).isoformat()
