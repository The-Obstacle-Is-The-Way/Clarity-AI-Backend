...
class AWSServiceFactory(AWSServiceFactoryInterface):
    ...
    def create_sagemaker_runtime(self) -> Any:
        ...
        try:
            ...
            raise ServiceConfigurationError(
                f"Failed to create SageMaker runtime client: {str(e)}",
    ...
    def create_sagemaker(self) -> Any:
        ...
        try:
            ...
            raise ServiceConfigurationError(
                f"Failed to create SageMaker client: {str(e)}",
...
class AWSXGBoostService(XGBoostInterface, Observable):
    ...
    def __init__(self, aws_factory: AWSServiceFactoryInterface) -> None:
        ...
        self._sagemaker_runtime: Optional[Any] = None
        self._sagemaker: Optional[Any] = None
    ...
    def initialize(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if isinstance(e, (ConfigurationError, ServiceConfigurationError)):
                raise
            raise ConfigurationError(f"Failed to initialize AWS XGBoost service: {str(e)}") from e
    ...
    def predict(
        ...
        **kwargs: Any
        ...
        try:
            self._notify_observers(EventType.PREDICTION_START, {
            ...
            self._notify_observers(EventType.PREDICTION_SUCCESS, {
                "patient_id": patient_id,
                "model_type": model_type,
                "result": result
            })
            ...
            self._notify_observers(EventType.PREDICTION_FAILURE, {
                "patient_id": patient_id,
                "model_type": model_type,
                "error": str(e)
            })
            ...
            raise PredictionError(
                f"Prediction failed for patient {patient_id}: {str(e)}",
                model_type=model_type
            ) from e
    ...
    def get_feature_importance(
        ...
        try:
            ...
            self._notify_observers(EventType.FEATURE_IMPORTANCE_COMPUTED, {
                "patient_id": patient_id,
                "model_type": model_type,
            ...
            if isinstance(e, ResourceNotFoundError):
                raise
            raise PredictionError(
                f"Failed to get feature importance: {str(e)}",
                model_type=model_type
            ) from e
        ...
        try:
            ...
            for endpoint in response.get("Endpoints", []):
                ...
                try:
                    ...
                    model_info["instance_type"] = endpoint_config["ProductionVariants"][0]["InstanceType"]
                except Exception:
    ...
    def _prepare_prediction_payload(
        ...
        try:
            ...
            raise SerializationError(
                f"Failed to serialize prediction payload: {str(e)}",
        ...
        try:
            ...
            if error_code == "ThrottlingException":
                ...
                raise ModelInvocationError(
                    f"Model error for endpoint {endpoint_name}: {str(e)}",
                    endpoint_name=endpoint_name,
                    status_code=e.response["Error"].get("HTTPStatusCode")
                ) from e
            else:
                raise ServiceConnectionError(
                    f"SageMaker error: {str(e)}",
                    service_name="sagemaker"
                ) from e
        except Exception as e:
            ...
            raise ModelInvocationError(
                f"Failed to invoke endpoint {endpoint_name}: {str(e)}",
                endpoint_name=endpoint_name
            ) from e
    ...
    def _extract_confidence(self, predictions: Any) -> Optional[float]:
    ...
    def _store_prediction_metadata(self, prediction_id: str, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to store prediction metadata: {str(e)}")

    def _get_prediction_metadata(self, prediction_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Failed to get prediction metadata: {str(e)}")
            return None
