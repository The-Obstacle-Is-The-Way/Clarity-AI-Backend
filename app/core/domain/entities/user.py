...
class User(BaseModel):
    ...
    reset_token_expires: datetime | None = Field(
        default=None, description="When the reset token expires"
    )
    ...
    def set_reset_token(self, token: str, expires: datetime | None = None) -> None:
        ...
        self.reset_token_expires = (
            expires if expires is not None else datetime.now() + timedelta(hours=24)
        )
    ...
    def is_reset_token_valid(self, token: str) -> bool:
        ...
        return (
            ...
            and self.reset_token_expires > datetime.now()
        )
