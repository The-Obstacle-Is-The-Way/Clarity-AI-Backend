...
class JWTPayloadBase(BaseModel):
    ...
    custom_fields: Dict[str, Any] = Field(default_factory=dict, description="Additional custom claims")
...
class RefreshTokenPayload(JWTPayloadBase):
    ...
    original_iat: Optional[int] = Field(None, description="Original token issued at timestamp")
...
def create_refresh_token_payload(
    ...
    issuer: Optional[str] = None,
    audience: Optional[str] = None,
    original_iat: Optional[int] = None,
    additional_claims: Optional[Dict[str, Any]] = None,
) -> RefreshTokenPayload:
    ...
    now = int(time.time()) if issued_at is None else int(issued_at.timestamp())
    exp_time = int(expires_at.timestamp()) if expires_at else now + (7 * 24 * 60 * 60)  # 7 days default
...
def payload_from_dict(data: Dict[str, Any]) -> JWTPayload:
    ...
    standard_claims = {
        "sub", "iat", "exp", "jti", "iss", "aud", "type", "roles", "role",
        "username", "session_id", "refresh", "original_iat"
    }
