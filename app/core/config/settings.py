...
class Settings(BaseSettings):
    ...
    def ensure_async_database_url(self) -> Self:
        ...
        if not self.ASYNC_DATABASE_URL and self.DATABASE_URL:
            ...
            logger.info(
                f"Set ASYNC_DATABASE_URL to {self.ASYNC_DATABASE_URL} based on DATABASE_URL"
            )
...
def get_settings() -> Settings:
    ...
    if os.environ.get("ENVIRONMENT") == "test" or os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        use_in_memory = os.environ.get("TEST_IN_MEMORY_DB", "True").lower() in (
            "true",
            "1",
            "yes",
        )
    ...
    if not current_settings.ASYNC_DATABASE_URL:
        ...
        if db_url.startswith("sqlite:///") and "aiosqlite" not in db_url:
            ...
            logger.info(
                f"Converted DATABASE_URL to async version: {current_settings.ASYNC_DATABASE_URL}"
            )
        else:
