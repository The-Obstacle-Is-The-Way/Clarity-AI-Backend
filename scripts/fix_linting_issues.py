...
def run_ruff_check(select: Optional[str] = None, fix: bool = False, 
                   path: Optional[str] = None, show_fixes: bool = True) -> Tuple[int, str]:
...
def run_black(check: bool = False, path: Optional[str] = None) -> Tuple[int, str]:
    """Run black formatter."""
    cmd = ["python", "-m", "black"]
...
def run_isort(check: bool = False, path: Optional[str] = None) -> Tuple[int, str]:
...
def fix_import_issues(path: Optional[str] = None) -> None:
...
def fix_formatting_issues(path: Optional[str] = None) -> None:
...
def fix_security_issues(path: Optional[str] = None) -> None:
...
def fix_exception_issues(path: Optional[str] = None) -> None:
...
def fix_unused_code(path: Optional[str] = None) -> None:
...
def fix_type_annotations(path: Optional[str] = None) -> None:
...
def generate_report() -> None:
    ...
    report["black"]["would_change"] = output.count("would reformat") if "would reformat" in output else 0
...
def main():
    ...
    parser.add_argument(
        "--path", 
        type=str, 
        help="Specific path to fix (default: entire app directory)"
    )
    parser.add_argument(
        "--report", 
        action="store_true",
        help="Generate a report of linting issues"
    )
