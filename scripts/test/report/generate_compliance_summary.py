...
class ComplianceSummaryGenerator:
    ...
    def generate_markdown_report(self) -> None:
        ...
        with open(report_path, "w") as f:
            ...
            if vulns:
                ...
                for vuln in vulns:
                    f.write(
                        f"| **{vuln['severity']}** | {vuln['description']} | {vuln['location']} | {vuln['recommendation']} |\n"
                    )
            else:
                f.write("No major vulnerabilities detected.\n\n")
    ...
    def display_console_report(self) -> None:
        ...
        if not RICH_AVAILABLE:
            print(
                "Rich library not available. Install with 'pip install rich' for better formatting."
            )
            return
        ...
        console.print(
            "[bold blue]           HIPAA COMPLIANCE SUMMARY REPORT                        "
        )
        console.print(
            "[bold blue]           Luxury Concierge Psychiatry Platform                   "
        )
        console.print(
            "[bold blue]==================================================================="
        )
        ...
        if recs:
            for i, rec in enumerate(recs, 1):
                ...
                console.print(
                    f"{i}. [bold]{rec['description']}[/bold] ([{priority_color}]{priority}[/{priority_color}])"
                )
        ...
        if status == "COMPLIANT":
            console.print(
                "[bold green]This platform meets HIPAA security and compliance requirements.[/bold green]"
            )
        else:
            console.print(
                "[bold red]This platform requires remediation to meet HIPAA requirements.[/bold red]"
            )
        console.print(
            "[bold blue]==================================================================="
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--reports-dir", default="reports", help="Directory containing security reports"
    )
