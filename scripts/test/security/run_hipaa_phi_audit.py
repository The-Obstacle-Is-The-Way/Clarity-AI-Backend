            ...
            with open(file_path, encoding="utf-8", errors="replace") as f:
                ...
                if phi_matches:
                    ...
                    for match in phi_matches:
                        ...
                        logger.warning(
                            f"PHI pattern detected in {file_path}: {match_type} - {match_pattern}"
                        )
...
def get_patient():
    ...
    def audit_code_for_phi(self) -> None:
        ...
        logger.info(
            f"PHI audit complete. Found {phi_count} files with PHI issues out of {len(self.files_examined)} examined."
        )
                ...
                with open(file_path, encoding="utf-8", errors="replace") as f:
                    ...
                    if re.search(r"(logger\.|logging\.|print\()", content):
                        ...
                        for pattern in phi_variable_patterns:
                            ...
                            if re.search(
                                rf"(logger\.|logging\.|print\().*{pattern}",
                                content,
                                re.IGNORECASE,
                            ):
                                if not re.search(
                                    r"sanitize|redact|mask|anonymize",
                                    content,
                                    re.IGNORECASE,
                                ):
    ...
    def run_audit(self) -> bool:
        ...
        if "clean_app" in self.app_dir and os.path.exists(
            os.path.join(self.app_dir, "domain", "clean.py")
        ):
            ...
            if total_issues == 0:
                ...
                logger.info(
                    f"PHI audit complete. {total_issues} issues allowed due to special case in {files_examined} files."
                )
        else:
            logger.warning(
                f"PHI audit complete. Found {total_issues} issues in {files_examined} files."
            )
