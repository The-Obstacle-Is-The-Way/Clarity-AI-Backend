...
"""
    ...
    try:
        ...
        if test_result.returncode == 0:
            print(
                f"✅ SUCCESS! All {passed_count} PHI audit tests PASSED in {test_duration:.2f} seconds!"
            )
            print("=" * 80)
            ...
            if (
                "test_ssn_pattern_detection" in test_result.stdout
                and "FAILED" in test_result.stdout
            ):
                print(
                    "- SSN pattern detection is not working as expected. Check the PHIDetector.detect_phi method."
                )
            if (
                "test_audit_report_generation" in test_result.stdout
                and "FAILED" in test_result.stdout
            ):
                print(
