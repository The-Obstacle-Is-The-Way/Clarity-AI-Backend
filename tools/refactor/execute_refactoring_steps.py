...
def determine_destination(file_path, base_dir):
    ...
    if (
        "trinity" in file_path.lower()
        or "mental_llama" in file_path.lower()
        or "pat" in file_path.lower()
        or "xgboost" in file_path.lower()
    ):
        return os.path.join(
            base_dir,
            "app",
    ...
    if (
        ...
        or "use_case" in file_path.lower()
        or "application" in file_path.lower()
    ):
        if "digital_twin" in file_path.lower():
            ...
            return os.path.join(
                base_dir, "app", "application", "use_cases", os.path.basename(file_path)
            )
...
def move_files(refactored_files, base_dir):
    ...
    for file_path in refactored_files:
        ...
        print(
            f"Moved: {os.path.relpath(file_path, base_dir)} -> {os.path.relpath(dest_path, base_dir)}"
        )
